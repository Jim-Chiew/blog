---
title: Photoprism - reset password
description:
draft: false
tags:
  - docker
created: 23/40/2025 22:40
updated: 23/19/2026 21:19
---
When installing photoprism. there will be 2 docker containers. to execute the command use the one without the `mariadb` database.
```bash
docker ps -a
...
List Users:
```bash
docker exec -it photoprism-photoprism-1 photoprism users ls
```

change password:
```bash
docker exec -it photoprism-photoprism-1 photoprism passwd admin
```
...
services:
  photoprism:
    ...
    volumes:
      ...
      - "/share/photoprism/Pictures:/photoprism/originals"               # Original media files (DO NOT REMOVE)
      # - "/example/family:/photoprism/originals/family" # *Additional* media folders can be mounted like this
      # - "~/Import:/photoprism/import"                  # *Optional* base folder from which files can be imported to originals
      - "/share/photoprism/storage:/photoprism/storage"                  # *Writable* storage folder for cache, database, and sidecar files (DO NOT REMOVE)
    ## NVIDIA GPU Hardware Acceleration (see https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/latest/install-guide.html):
    # deploy:
    #  resources:
    #    reservations:
  ...
  ollama:
    ...
    volumes:
      - "/share/photoprism/ollama:/root/.ollama"
    ## NVIDIA GPU Hardware Acceleration (see https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/latest/install-guide.html):
    # deploy:
    #  resources:
    #    reservations:
  ...
  open-webui:
    ...
    volumes:
      - "/share/photoprism/open-webui:/app/backend/data"
